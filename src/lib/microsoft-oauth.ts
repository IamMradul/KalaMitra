...
export async function getMicrosoftUserInfo(accessToken: string): Promise<MicrosoftUser> {
  ...
  // Get user photo (optional, may not always be available)
  // Microsoft Graph API doesn't provide a direct public URL for profile photos
  // Users can upload their profile picture later through the app
  // To fetch photos, you would need to:
  // 1. Add 'User.ReadBasic.All' or 'User.Read.All' permission
  // 2. Fetch the photo as binary data and store it in your storage
  // For now, we'll leave it as null
  const picture: string | null = null

  return {
    id: profile.id,
    email: profile.mail || profile.userPrincipalName,
    name: profile.displayName || profile.givenName + ' ' + profile.surname || profile.userPrincipalName,
    picture: picture,
